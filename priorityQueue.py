...
#Goal: Implement a simple priority queue_list. Assume an incoming stream of dictionaries containing two keys:
...
class PriorityQueue:
    ...
    def push(self, priority_command_dict):
        ...
        @param priority Integer value between 0 and 10
        ...
        for i in range(len(self.queue_list)):
            ...
            # the item's priority is greater than the current priority so insert it
                ...
                self.queue_list.insert(i, (priority, priority_command_dict['command']))
                return
    ...
    def pop(self):
        ...
        if self.queue_list:
            ...
            self.queue_list = [(p, c) for (p, c) in self.queue_list[1:]]
            return command
        return None
